// Simple Supabase connection        if (sessionError) {
            console.error('❌ Auth connection failed:', sessionError.message)
            return false
        }
        
        console.log('✅ Auth connection successful!')
        console.log('👤 Current session:', sessionData.session ? 'Active' : 'No session (expected)')port { createClient } from '@supabase/supabase-js'
...
async function testConnection() {
    try {
        ...
        if (error) {
            ...
            console.log('💡 This might mean your database is not accessible or the keys are wrong')
            return false
        }
        
        console.log('✅ Connection successful!')
        console.log('�️  Database version:', data || 'PostgreSQL')
        ...
        if (clientsError) {
            ...
            console.log('   3. Run the contents of database/supabase-schema.sql')
